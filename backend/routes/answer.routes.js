...
router.post('/:questionId/answers', authMiddleware.authUser, upload.array('images', 5), [
  body("body").trim().notEmpty().withMessage("Body is required").isLength({ max: 5000 }).withMessage("Max 5000 characters"),
], answerController.createAnswer);

router.put('/:questionId/answers/:id', authMiddleware.authUser, upload.array('images', 5), [
  body("body").trim().notEmpty().withMessage("Body is required").isLength({ max: 5000 }).withMessage("Max 5000 characters"),
